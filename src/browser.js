...
class RawFileBrowser extends React.Component {
  ...
  componentDidMount() {
    ...
    this.getLocale(language).then(locale => {
      this.setState({
        locale,
      })
    })
  ...
  render() {
    ...
    switch (this.props.renderStyle) {
        ...
        if (!contents.length) {
          if (this.state.nameFilter) {
            contents = (
              <tr>
                <td colSpan={100}>
                  {messages.no_files_matching} "{this.state.nameFilter}".
                </td>
              </tr>
            )
          } else {
      ...
      case 'list':
        if (!contents.length) {
          if (this.state.nameFilter) {
            contents = (<p className="empty">{messages.no_files_matching} "{this.state.nameFilter}"</p>)
          } else {
            contents = (<p className="empty">{messages.no_files}</p>)
          }
        } else {
          let more
          if (this.state.nameFilter) {
